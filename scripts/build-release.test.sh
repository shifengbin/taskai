...
grep -F "(Join-Path \${env:ProgramFiles(x86)} 'NSIS\\makensis.exe')" "$workflow" >/dev/null
grep -F '(Split-Path -Parent $MakeNSIS) | Out-File -FilePath $env:GITHUB_PATH -Encoding utf8 -Append' "$workflow" >/dev/null
