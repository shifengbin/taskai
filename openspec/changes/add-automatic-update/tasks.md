...
- [ ] 9.1 用户确认后，将功能 worktree 分支合并到当前工作区项目对应分支；发生冲突时保留双方有效变更并完成冲突测试
- [ ] 9.2 在合并后的当前工作区重新执行 `go test -race ./...`、前端测试与构建、发布脚本测试，并重新编译当前平台可执行程序验证合并结果
- [ ] 9.3 同步 README、设计记录和必要的发布说明，核对 OpenSpec 需求与任务均已满足后归档 `add-automatic-update` 变更
- [ ] 9.4 提交合并、实现、测试与文档变更，确认工作区干净且提交包含归档结果
- [ ] 9.5 移除已经合并的 `.worktrees/add-automatic-update` worktree，并清理对应已合并功能分支
