...
- [x] 2.2 在 `main` 包定义可由 linker flag 注入且具有明确开发默认值的 `appVersion`，并让 Linux、Windows、macOS 构建脚本向可执行程序注入同一规范化版本
...
- [x] 3.2 实现 Release/Manifest/Asset/State 类型、当前平台键映射和官方 GitHub Release 客户端，只允许同一官方 Release 中与清单匹配的资产成为下载候选
...
- [x] 4.3 先增加失败测试，覆盖重启后有效缓存恢复、损坏缓存删除、当前或旧版本缓存清理，以及每次恢复都重新计算摘要
...
- [x] 6.2 在前端 API 和类型层接入生成的更新方法与 `updater:state-changed` 事件，确保启动检查早于订阅时仍能通过初始查询恢复状态
