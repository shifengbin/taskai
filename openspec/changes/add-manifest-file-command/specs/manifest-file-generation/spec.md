...
系统 MUST 优先使用 Git 信息中非空的 `branch` 值。当 Git 信息的 `branch` 为空白时，系统 MUST 使用当前任务模板定义下任务保存的 `branch` 字段值；两者均为空时，系统 MUST 将该仓库的 `branch` 写为空字符串，而不得省略该键。

#### Scenario: 使用任务模板分支回填空 Git 分支
- **WHEN** 任务模板的 `branch` 值为 `android2.45-0727`，且一个已选 Git 信息的 `branch` 为空
- **THEN** 生成清单中该仓库的 `branch` 为 `android2.45-0727`

#### Scenario: 保留 Git 信息的显式分支
- **WHEN** 任务模板的 `branch` 值为 `android2.45-0727`，且一个已选 Git 信息的 `branch` 为 `dev-cj-1.2`
- **THEN** 生成清单中该仓库的 `branch` 为 `dev-cj-1.2`

#### Scenario: 输出空分支
- **WHEN** Git 信息和任务模板的 `branch` 均为空
