...
系统 MUST 要求最终任务工作目录存在、为目录且自身不是符号链接或其他重解析点。系统 MUST 允许该目录的上级路径由操作系统通过重解析点正常解析。系统 MUST 在已验证的任务工作目录内安全创建缺失的合法 `dir` 子目录，并拒绝经过符号链接或其他重解析点、离开任务工作目录或指向目录、符号链接、其他重解析点及其他非普通文件的目标。对不存在或为普通文件的目标，目标验证 MUST 不阻止写入继续进行。目标为普通文件时，系统 MUST 以原子替换方式覆盖其内容，且写入失败时不得留下部分清单内容。
...
#### Scenario: 在 Windows 上写入不存在的普通清单目标
- **WHEN** Windows 任务工作目录是普通目录，且清单目标此前不存在
- **THEN** 系统在该任务工作目录内生成清单文件，且目标验证步骤不会因目标不存在或其所在平台而拒绝该普通目标
