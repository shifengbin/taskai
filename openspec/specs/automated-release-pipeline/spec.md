# automated-release-pipeline Specification

## Purpose
定义在推送 `v*` tag 或手动触发时，于三个原生运行器并行构建 Linux、Windows、macOS 发布产物，按 tag 派生统一版本号、上传产物，并仅在 tag 触发时发布 GitHub Release 的自动化发布管线行为。
## Requirements
