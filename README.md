...
- 执行中的任务可从右键菜单或“任务操作”下拉菜单新增多个终端。终端作为任务树子节点显示，并在右侧使用 xterm 交互。
...
- 工作区根目录、任务树宽度、亮色/暗色模式、终端 Shell 路径和任务操作菜单可持久化设置；设置界面会探测当前平台可用的 Shell 供下拉选择，也支持手动填写有效路径。修改根目录只影响后续任务。
