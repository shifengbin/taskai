...
- 支持可复用的生命周期命令链。设置中的“生命周期编排”可为自定义命令和命令链勾选一个或多个适用钩子；链只能加入覆盖其全部范围的命令，默认链和新建任务下拉框也只显示当前钩子适用的链。未执行任务编辑时可调整链选择；执行中和已完成任务只读展示链选择。系统内置“创建任务工作目录”固定仅用于 `beforeStart`，“删除任务工作目录”固定仅用于 `postEnd`，两者不可修改或删除。
...
命令链按配置顺序执行，全部通过任务设置中选定的 Shell 启动。第一条命令的 UTF-8 标准输入是任务 HTTP 详情同结构的 JSON；当本机 HTTP 服务已启用时，顶层额外包含 `baseURL`。每一条后续命令的标准输入严格等于上一条命令的标准输出原始字节，不会自动解析、合并或补回任务参数，因此命令应自行输出下一条命令需要的内容。
