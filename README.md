...
生命周期命令的“固定参数”属于命令本身，每次执行都会传入。新增或编辑自定义命令时可选择“允许在命令链中追加参数”；关闭时，命令链编辑器不会显示该命令的追加参数框，开启时才可逐行配置每个链引用的追加参数。关闭开关不会清空或改写已保存的链级追加参数，重新开启后会恢复显示；无论开关当前状态如何，既有链执行时都会先传入固定参数，再传入已保存的追加参数。系统“创建任务工作目录”和“删除任务工作目录”固定禁止追加参数，“Git 仓库克隆”固定允许。
