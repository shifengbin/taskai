...
- 工作区根目录、任务树宽度、亮色/暗色模式、终端 Shell 路径和任务操作菜单可持久化设置；设置使用“工作区与外观 / 终端 Shell / 任务操作”三个 Tab。新增或编辑自定义菜单项会在独立弹窗中完成，其中“前后置脚本”Tab 的“？”入口说明标准输入 JSON 字段和参数传递规则；确认主设置前不会写入磁盘。设置界面会探测当前平台可用的 Shell 供下拉选择，也支持手动填写有效路径。修改根目录只影响后续任务。
