...
- 顶部“额外信息管理”可独立新建和删除分类；分类下仍有信息模板时必须先删除这些模板。模板从已有分类中选择，包含信息展示名称、多个固定字段（键、显示名称、值）和多个可选或必填参数；创建或编辑任务时按分类筛选并勾选信息，切换分类会保留已选项，顶部的方形标签可通过 `x` 移除。任务会保存完整快照，因此之后修改或删除模板、分类不会影响已有任务。
...
列表返回任务数组；详情返回单个任务，包含 `id`、`title`、`description`、`color`、`status`、创建/完成时间以及工作区根目录和任务工作目录。详情额外包含 `extraInfo`：它按 `catalogue` 聚合为对象键，每项将全部固定字段的键和值及参数键、参数值平铺为对象；列表接口不返回该字段。无效 `status` 返回 `400`，不存在的任务返回 `404`。

例如任务选择两个 Git 仓库后，详情中的附加信息为：
...
```json
{
  "extraInfo": {
    "git": [
      ...
      {"repository": "git@example.com:team/web.git", "branch": "release/1.0"}
    ]
  }
}
```

`extraInfo` 不返回信息展示名称、固定字段显示名称、参数显示名称或元素内独立的 `catalogue` 字段；任务未选择附加信息时返回空对象 `{}`。
