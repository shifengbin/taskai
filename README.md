...
- 执行中的任务可从右键菜单或“任务操作”下拉菜单新增多个终端。终端作为任务树子节点显示，并在右侧使用 xterm 交互；双击任务条目可展开或收起终端，任务可在同一状态标签页内拖动排序并持久化。
- 任务操作菜单可在设置中排序；固定操作仅可调序，自定义操作可配置名称、命令、逐行参数和是否显示为独立终端。自定义命令会通过已选择 Shell 的初始化环境启动，因此可直接使用该 Shell `PATH` 中的 `codex`；未显示终端的命令会在任务工作目录中后台启动，例如 `code .`。
- 工作区根目录、任务树宽度、亮色/暗色模式、终端 Shell 路径和任务操作菜单可持久化设置；设置按“工作区与外观 / 终端 Shell / 任务操作菜单”分区，新增或编辑自定义菜单项会在独立弹窗中完成，确认主设置前不会写入磁盘。设置界面会探测当前平台可用的 Shell 供下拉选择，也支持手动填写有效路径。修改根目录只影响后续任务。
