...
- 支持可复用的生命周期命令链。设置中的“生命周期编排”可为自定义命令和命令链勾选一个或多个适用钩子；链只能加入覆盖其全部范围的命令，默认链和新建任务下拉框也只显示当前钩子适用的链。未执行任务编辑时可调整链选择；执行中和已完成任务只读展示链选择。系统内置“创建任务工作目录”固定仅用于 `beforeStart`，“删除任务工作目录”固定仅用于 `postEnd`；“Git 仓库克隆”可用于 `beforeStart`、`beforeEnd` 和 `updateTask`。三者均不可修改或删除。
...
命令链按配置顺序执行。自定义命令通过任务设置中选定的 Shell 启动；系统目录命令和“Git 仓库克隆”由应用直接执行。第一条自定义命令的 UTF-8 标准输入是任务 HTTP 详情同结构的 JSON；当本机 HTTP 服务已启用时，顶层额外包含 `baseURL`。每一条后续自定义命令的标准输入严格等于上一条命令的标准输出原始字节，不会自动解析、合并或补回任务参数，因此命令应自行输出下一条命令需要的内容。

`beforeStart` 在任务状态变为“执行中”前运行，`beforeEnd` 在状态变为“已完成”前运行；两者失败时状态不会改变。`postStart`、`postEnd` 和执行中任务更新后的 `updateTask` 在状态或内容保存后运行，失败不会回滚已提交的状态或内容。链执行或失败等待重试期间，任务树会显示当前钩子、命令和序号，并锁定编辑、排序、状态变更、右键菜单及任务操作；点击“重试命令链”会始终从链的第一条命令重新开始。

默认配置把“创建任务工作目录”放在 `beforeStart`，把“删除任务工作目录”放在 `postEnd`。内置创建操作可安全复用已存在的任务目录，删除不存在目录也视为成功。除 `postEnd` 外，命令都在任务工作目录中执行；`postEnd` 在工作区根目录中执行。若已完成任务引用的链后来被删除，界面仍会保留失败记录；重试时会提示“命令链已删除”。

命令和链可勾选多个适用钩子，但链内每条命令都必须覆盖链勾选的全部钩子。若链已被未执行或执行中任务选择，保存时不得移除这些任务所需的范围；这样既有任务在不可编辑链选择的前提下仍保持可执行。旧配置中的自定义命令和链会自动迁移为五个钩子均适用。

### Git 仓库克隆

在任务中选择一个或多个内置 Git 信息后，可在“生命周期编排”中把系统命令“Git 仓库克隆”加入命令链。命令链对该命令的参数必须逐行填写且只能填写一行：`dir=<相对目录>`。`dir` 相对于任务工作目录，`.` 表示直接放在任务工作目录中；不接受绝对路径、空目录、`..` 或会跳出任务工作目录的路径。

建议在 `beforeStart` 链中先放置“创建任务工作目录”，再放置“Git 仓库克隆”，例如为 Git 命令配置 `dir=repositories`。应用会依次处理所选 Git 信息，并把每个项目克隆到 `<任务工作目录>/<dir>/<项目名称>`。目标路径已经存在（无论是文件还是目录）时会跳过该项目；因此在某个仓库失败后重试同一命令链，会保留并跳过已经成功克隆的仓库。

克隆时直接调用本机 `git`，不会通过任务选定的 Shell 解释仓库地址、分支或目录。未填写分支时使用远程默认分支；填写的分支在远程存在时直接检出；远程不存在该分支时，先克隆远程默认分支，再以它为基础创建同名本地分支。Git 命令不会自动加入默认链；除推荐的 `beforeStart` 外，也可按需要配置在 `beforeEnd` 或 `updateTask` 链中。
