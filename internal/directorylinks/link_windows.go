...
func (nativeDirectoryLinkFS) Create(linkPath, targetPath string) error {
	...
	if err := os.Symlink(targetPath, linkPath); err != nil {
		return fmt.Errorf("当前 Windows 环境无法创建安全目录链接 %q -> %q: %w", linkPath, targetPath, err)
	}
	return nil
}
...
func (nativeDirectoryLinkFS) Read(linkPath string) (string, bool, error) {
	...
	if info.Mode()&os.ModeSymlink == 0 {
		return "", false, fmt.Errorf("路径不是受支持的 Windows 目录链接: %q", linkPath)
	}
	target, err := os.Readlink(linkPath)
	if err != nil {
		return "", false, fmt.Errorf("读取 Windows 目录链接失败 %q: %w", linkPath, err)
	}
...
func (nativeDirectoryLinkFS) Remove(linkPath string) error {
	...
	if err := os.Remove(linkPath); err != nil {
		return fmt.Errorf("删除 Windows 目录链接失败 %q: %w", linkPath, err)
	}
	return nil
}
