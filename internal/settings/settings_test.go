...
func TestTaskMenuItemIgnoresLegacyMouseClipboardPolicy(t *testing.T) {
	var legacy TaskMenuItem
	if err := json.Unmarshal([]byte(`{"id":"custom-legacy","kind":"command","name":"Legacy","command":"legacy","showTerminal":true,"disableTaskAIMouseClipboard":true}`), &legacy); err != nil {
	...
	if _, exists := persisted["disableTaskAIMouseClipboard"]; exists {
		t.Fatalf("再次保存仍包含旧鼠标策略字段: %s", contents)
	}
}
